...
@onready var _surface_texture: OptionButton = $SurfaceTexture
...
func _on_world_texture_item_selected(index) -> void:
	SaveManager.settings.set_data("video_world", "surface_texture", _surface_texture.get_item_metadata(index))
...
func _load_world_textures() -> void:
	...
	for path: String in Global.get_world_textures():
		_surface_texture.add_icon_item(CustomResourceManager.get_image(path), path.get_file(), i)
		_surface_texture.set_item_metadata(i, path)
		i+=1

func _get_selected_texture_index() -> void:
	var selected = SaveManager.settings.get_data("video_world", "surface_texture")
	for i in range(_surface_texture.item_count):
		if selected == _surface_texture.get_item_metadata(i):
			_surface_texture.select(i)
